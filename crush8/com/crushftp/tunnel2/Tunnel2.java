...
public class Tunnel2 {
  ...
  public void startThreads() {
    while (true) {
      try {
        checkStatus();
        if (this.tunnel == null || this.tunnel.size() == 0 || this.tunnel.getProperty("tunnel_version", "tunnel2").equalsIgnoreCase("tunnel3"))
          break; 
  ...
  public void checkStatus() throws Exception {
    ...
    synchronized (this) {
      if (this.auth == null) {
        ...
        this.auth = Common.login(getUrl(true), this.username, this.password, "CrushTunnel");
      } 
      if (this.tunnel == null)
  ...
  public String stopThisTunnel() {
    msg("Tunnel is stopping...locals=" + this.local.size() + ", remotes=" + this.remotes.size() + ", queues=" + this.queues.size() + ", threads=" + ((this.ch != null) ? this.ch.threads.size() : 0));
    this.ready = false;
    setActive(false);
    this.queues.clear();
  ...
  public synchronized String getUrl(boolean toggle) {
    ...
    msg("Returning URL:" + (new VRL(s)).safe());
    return s;
  }
