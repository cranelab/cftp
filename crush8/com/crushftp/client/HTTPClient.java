  ...
  public String login2(String username, String password, String clientid) throws Exception {
    ...
    if (u.getProtocol().equalsIgnoreCase("HTTPS") && !this.config.getProperty("keystore_path", "").equals("")) {
      ...
      try {
        ...
        if (!getConfig("SAMLResponse", "").equals(""))
          uRLConnection.getOutputStream().write(("&SAMLResponse=" + getConfig("SAMLResponse")).getBytes("UTF8")); 
        uRLConnection.setReadTimeout(Integer.parseInt(this.config.getProperty("timeout", "30000")));
    ...
    try {
      ...
      if (!getConfig("SAMLResponse", "").equals(""))
        urlc.getOutputStream().write(("&SAMLResponse=" + getConfig("SAMLResponse")).getBytes("UTF8")); 
      urlc.setReadTimeout(Integer.parseInt(this.config.getProperty("timeout", "30000")));
  ...
  public String doAction(String command, String param1, String param2) throws Exception {
    ...
    while (loops++ < 6) {
      ...
      if (!command.equalsIgnoreCase("list") && !command.equalsIgnoreCase("agentQueue"))
        if (result.contains("url=") && result.indexOf(";", result.indexOf("url=")) >= 0) {
          String url = result.substring(result.indexOf("url=") + "url=".length(), result.indexOf(";", result.indexOf("url=")));
          log(Common.replace_str(result, url, (new VRL(Common.url_decode(url))).safe()));
        } else {
          log(result);
        }  
      return result;
