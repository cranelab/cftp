...
public class URLConnection {
  ...
  public void connect() throws IOException {
    ...
    if (socks == null)
      socks = new Vector(); 
    while (true) {
      this.sock = Common.getSocket(this.config.getProperty("protocol", "DAV"), this.u, this.config.getProperty("use_dmz", "false"), this.config.getProperty("crushAuth", ""), Integer.parseInt(this.config.getProperty("timeout", "30000")));
      try {
        if ((this.u.getProtocol().equalsIgnoreCase("HTTPS") && !(this.sock instanceof SSLSocket)) || (this.u.getProtocol().equalsIgnoreCase("WEBDAVS") && !(this.sock instanceof SSLSocket)))
          if (!this.u.getHost().equals("127.0.0.1") || System.getProperty("crushftp.dmz.ssl", "true").equals("true")) {
            ...
            if (last_cipher != null && Thread.currentThread().getName().indexOf(last_cipher) < 0)
        ...
        if (this.remove_double_encoding)
          tmp_path = remove_double_encoding_of_special_chars(tmp_path); 
  ...
  private void closeHeaders() throws IOException {
    ...
    if (this.expect100) {
      ...
      Common.log("HTTP_CLIENT", 2, "Expect: 100-continue");
    } 
  ...
  public void setReadTimeout(int read_timeout) throws IOException {
    this.config.put("timeout", (new StringBuffer(String.valueOf(read_timeout))).toString());
    if (this.sock != null)
  ...
  public static String consumeResponse(InputStream in) throws IOException {
    return consumeResponse(in, true);
  }
