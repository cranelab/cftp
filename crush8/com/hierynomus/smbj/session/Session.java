...
public class Session implements AutoCloseable {
  ...
  public Share connectShare(String shareName) {
    if (shareName.contains("\\"))
      throw new IllegalArgumentException(String.format("Share name (%s) cannot contain '\\' characters.", new Object[] { shareName })); 
  ...
  public void logoff() throws TransportException {
    try {
      ...
      for (Session nestedSession : this.nestedSessions) {
        logger.info("Logging off nested session {} for session {}", Long.valueOf(nestedSession.getSessionId()), Long.valueOf(this.sessionId));
        try {
          nestedSession.logoff();
        } catch (TransportException te) {
          logger.error("Caught exception while logging off nested session {}", Long.valueOf(nestedSession.getSessionId()));
        } 
      } 
