  ...
  public void setRecipients(Message.RecipientType type, String addresses) throws MessagingException {
    if (type == RecipientType.NEWSGROUPS) {
      ...
      setAddressHeader(getHeaderName(type), (addresses == null) ? null : 
          (Address[])InternetAddress.parse(addresses));
    } 
  }
    ...
    if (replyToAll) {
      ...
      if (alternates != null)
        eliminateDuplicates(v, 
            (Address[])InternetAddress.parse(alternates, false)); 
  ...
  private Address[] eliminateDuplicates(List<Address> v, Address[] addrs) {
    ...
    if (gone != 0) {
      ...
      if (addrs instanceof InternetAddress[]) {
        InternetAddress[] arrayOfInternetAddress = new InternetAddress[addrs.length - gone];
      } else {
