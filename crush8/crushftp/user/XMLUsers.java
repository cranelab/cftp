...
public class XMLUsers extends UserProvider {
  ...
  public static String findUser(String path, String username) {
    ...
    for (x = 0; list != null && x < list.length; x++) {
      if (list[x].isDirectory())
        if (System.getProperty("crushftp.case_sensitive_user", "false").equals("true")) {
          if (list[x].getName().equals(username))
            return String.valueOf(list[x].getPath()) + "/"; 
        } else if (list[x].getName().equalsIgnoreCase(username)) {
          return String.valueOf(list[x].getPath()) + "/";
        }  
    } 
  ...
  public static CIProperties buildVFSXML(String vfsHome) {
    ...
    Properties permisions0_new = new Properties();
    ...
    if (!(new File_S(String.valueOf(vfsHome) + "VFS/")).exists()) {
  ...
  public Properties loadGroups(String serverGroup) {
    ...
    if ((new File_S(groupsPath)).exists()) {
      try {
  ...
  public void writeVFS(String serverGroup, String username, Properties virtual) {
    ...
    try {
      ...
      (new File_S(String.valueOf(getUserPath(serverGroup, username)) + "VFS/")).mkdir();
      ...
      while (keys.hasMoreElements()) {
        ...
        if (p.getProperty("type").equalsIgnoreCase("DIR")) {
          (new File_S(String.valueOf(getUserPath(serverGroup, username)) + "VFS" + virtualPath)).mkdirs();
          continue;
        } 
        (new File_S(String.valueOf(getUserPath(serverGroup, username)) + "VFS" + Common.all_but_last(virtualPath))).mkdirs();
  ...
  public Properties loadInheritance(String serverGroup) {
    ...
    if ((new File_S(inheritancePath)).exists()) {
      try {
  ...
  public Properties loadUser(String serverGroup, String username, Properties inheritance, boolean flattenUser) {
    ...
    Properties newUser = read_user_no_cache(default_server_group, "default");
    if (user == null)
      return null; 
  ...
  public void updateUser(String serverGroup, String username1, String username2, String password) {
    if (username2 != null) {
      File_S rnfr = new File_S(getUserPath(serverGroup, username1));
      File_S rnto = new File_S(getUserPath(serverGroup, username2));
      rnfr.renameTo(rnto);
    } 
  }
  ...
  public void writeUser(String serverGroup, String username, Properties user, boolean backup) {
    ...
    if (!is_newUser)
      try {
        if (backup) {
          writeBackupUser(serverGroup, username, user_dir);
          writeExtraVFSBackup(username);
        } 
      } catch (Exception ee) {
        Log.log("USER_OBJ", 0, ee);
      }  
    ...
    if (is_newUser) {
      user_dir = getUserPath(serverGroup, username);
      try {
  ...
  private void writeExtraVFSBackup(String username) throws Exception {
    ...
    for (int x = 0; x < folders.length; x++) {
      ...
      if (f.getName().startsWith(String.valueOf(username) + "~"))
        writeBackupUser("extra_vfs", f.getName(), getUserPath("extra_vfs", f.getName())); 
    } 
  }
  ...
  private void writeBackupUser(String serverGroup, String username, String path) throws Exception {
    ...
    for (int x = zipFiles.size() - 1; x >= 0; x--) {
      ...
      } else if (((File_S)zipFiles.elementAt(x)).length() > 1048576L) {
        zipFiles.removeElementAt(x);
      } 
    } 
  ...
  public Properties read_user_no_cache(String serverGroup, String username) {
    ...
    try {
      ...
      if (common == null)
        common = new Common(); 
      Properties p = null;
      for (int x = 0; p == null && x < 5 && (new File_S(real_path)).exists(); x++) {
        try {
          synchronized (user_lock) {
  ...
  public void addFolder(String serverGroup, String username, String path, String name) {
    ...
    if (path.equals("/"))
      path = ""; 
    (new File_S(String.valueOf(getUserPath(serverGroup, username)) + "VFS/" + path + name)).mkdirs();
