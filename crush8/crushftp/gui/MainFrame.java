...
public class MainFrame extends JFrame {
          ...
          public void actionPerformed(ActionEvent arg0) {
            if (ServerStatus.SG("max_max_users").equals("5")) {
              ...
              if (Common.machine_is_windows()) {
                ...
                String ram_megabytes = (String)JOptionPane.showInputDialog(this.this$0.thisObj, "Max RAM for server in MegaBytes:", "Max RAM for Server", 0, null, null, "512");
                if (ram_megabytes != null) {
                  if (!ServerStatus.thisObj.common_code.install_windows_service(Integer.parseInt(ram_megabytes.trim()))) {
                  ...
                  if (response == 0) {
                    boolean ok = false;
                    while (!ok) {
                      String domainuser = (String)JOptionPane.showInputDialog(this.this$0.thisObj, "Windows Domain Username:", "Windows Domain Username", 0, null, null, ".\\smithbob");
                      if (domainuser != null) {
                        String domainpass = Common.getPasswordPrompt("Password:");
                        String result = ServerStatus.thisObj.common_code.install_windows_service_username(domainuser, domainpass);
                        if (result.equals("")) {
                          this.this$0.stopDaemon(true);
                          this.this$0.startDaemon(true);
  ...
  public void stopDaemon(boolean silent) {
    ...
    try {
      ...
      if (Common.machine_is_windows()) {
        ...
        if (Common.test_elevate()) {
          Common.writeFileFromJar("service/elevate.exe", "./", true);
          proc = Runtime.getRuntime().exec(new String[] { (new File_S("./service/elevate.exe")).getCanonicalPath(), "-c", "-w", "net", "stop", "CrushFTP Server" }, (String[])null, new File_S("./service/"));
        } else {
          proc = Runtime.getRuntime().exec(new String[] { "net", "stop", "CrushFTP Server" }, (String[])null, new File_S("./service/"));
        } 
      } else if (Common.machine_is_x()) {
        RandomAccessFile out = new RandomAccessFile(new File_S("crushftp_exec_root.sh"), "rw");
        out.setLength(0L);
        out.write("launchctl stop com.crushftp.CrushFTP\n".getBytes("UTF8"));
        out.close();
        Common.exec(new String[] { "chmod", "+x", (new File_S("crushftp_exec_root.sh")).getCanonicalPath() });
        Common.check_exec();
        proc = Runtime.getRuntime().exec(new String[] { "osascript", "-e", "do shell script \"" + (new File_S("crushftp_exec_root.sh")).getCanonicalPath() + "\" with administrator privileges" });
      } 
    ...
    if ((new File_S("crushftp_exec_root.sh")).exists())
      (new File_S("crushftp_exec_root.sh")).delete(); 
  ...
  public void startDaemon(boolean silent) {
    ...
    try {
      ...
      if (Common.machine_is_windows()) {
        if (Common.test_elevate()) {
          Common.writeFileFromJar("service/elevate.exe", "./", true);
          proc = Runtime.getRuntime().exec(new String[] { (new File_S("./service/elevate.exe")).getCanonicalPath(), "-c", "-w", "net", "start", "CrushFTP Server" }, (String[])null, new File_S("./service/"));
        } else {
          proc = Runtime.getRuntime().exec(new String[] { "net", "stop", "CrushFTP Server" }, (String[])null, new File_S("./service/"));
        } 
      } else if (Common.machine_is_x()) {
  ...
  public void checkAuthed() {
    if (Common.machine_is_x_10_6_plus()) {
    ...
    if (!Common.OSXApp())
      this.btnAuthenticateCrushftpFor.setEnabled(false); 
    if (Common.machine_is_x()) {
      this.btnInstallDaemon.setEnabled(Common.OSXApp());
      this.btnRemoveDaemon.setEnabled(Common.OSXApp());
      this.btnStartDaemon.setEnabled(Common.OSXApp());
      this.btnStopDaemon.setEnabled(Common.OSXApp());
      if (!Common.OSXApp()) {
