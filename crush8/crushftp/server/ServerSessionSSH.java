...
public class ServerSessionSSH implements FileSystem {
  ...
  public Object closeFileLock = new Object();
  ...
  public void init(byte[] sessionid, SessionChannel session, SshContext context, String protocolInUse) throws IOException {
    ...
    if (minutes == 0)
      minutes = 9999; 
    this.thread_killer_item = new IdleMonitor(this.thisSession, (new Date()).getTime(), minutes, null);
  ...
  public SftpFileAttributes getFileAttributes(Properties item) {
    ...
    if (this.thisSession.DG("timezone_offset") != 0.0D) {
      Date d = new Date(lastMod);
      cal.setTime(d);
      cal.setTimeInMillis((long)(cal.getTimeInMillis() + this.thisSession.DG("timezone_offset") * 1000.0D * 60.0D * 60.0D));
      lastMod = cal.getTime().getTime();
    } 
    if (lastMod <= 0L)
      lastMod = (new Date()).getTime(); 
    attrs.setTimes(new UnsignedInteger64(lastMod / 1000L), new UnsignedInteger64(lastMod / 1000L));
    return attrs;
  }
  ...
  public byte[] openDirectory(String path) throws PermissionDeniedException, FileNotFoundException, IOException {
    ...
    try {
      ...
      Properties p = new Properties();
      p.put("command_code", "0");
      p.put("command_data", (new StringBuffer(String.valueOf(result))).toString());
      this.thisSession.runPlugin("afterCommand", p);
    } 
  ...
  public byte[] openFile(String path, UnsignedInteger32 flags, SftpFileAttributes attrs) throws PermissionDeniedException, FileNotFoundException, IOException {
    synchronized (this.statLock) {
      ...
      try {
        ...
        if ((flags.intValue() & 0x8) == 8 || (flags.intValue() & 0x2) == 2 || (flags.intValue() & 0x10) == 16 || (flags.intValue() & 0x4) == 4) {
          this.thisSession.uiPUT("the_command_data", path);
          ...
          if ((flags.intValue() & 0x8) == 8 || (flags.intValue() & 0x10) == 16) {
            if (item != null) {
              ...
              try {
                ...
                item.put("type", "FILE");
                item.put("modified", (new StringBuffer(String.valueOf((new Date()).getTime()))).toString());
                Calendar cal = Calendar.getInstance();
                cal.setTime(new Date());
                item.put("time_or_year", (new StringBuffer(String.valueOf(cal.get(1)))).toString());
              } catch (Exception exception) {}
            } 
      ...
      try {
        if (item.getProperty("allow_write", "false").equals("true")) {
          ...
          if (outStream == null) {
            ...
            if (Long.parseLong(item.getProperty("size", "0")) != offset.longValue())
  ...
  public void closeFileMsg(byte[] handle, String error_msg) throws InvalidHandleException, IOException {
    ...
    delay();
    path = fixPath(path);
    synchronized (this.closeFileLock) {
  ...
  public void removeFile(String path) throws PermissionDeniedException, IOException, FileNotFoundException {
    ...
    delay();
    path = fixPath(path);
    if (!getFileAttributes(path).isFile()) {
      this.quickLookupDirItem.clear();
  ...
  public void closeFilesystem() {
    ...
    while (keys.hasMoreElements()) {
      ...
      if (item != null)
        try {
          closeFileMsg(key.getBytes(), "SSH Session disconnected while transfer in progress!");
        } catch (Exception e) {
          Log.log("SSH_SERVER", 1, e);
        }  
    } 
