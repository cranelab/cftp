...
public class ServerSessionHTTPWI {
  public static File_B serveFile(ServerSessionHTTP sessionHTTP, Vector header, OutputStream original_os, boolean onlyGetFile, String downloadFilename) throws Exception {
    ...
    for (int x = 0; x < header.size(); x++) {
      ...
      if (data.startsWith("Accept-Encoding: ")) {
        ...
        if (data.toUpperCase().indexOf("GZIP") >= 0 && ServerStatus.BG("allow_gzip"))
          acceptsGZIP = true; 
      } else if (x == 0 && (data.startsWith("HEAD ") || data.startsWith("GET ")) && data.indexOf(" /WebInterface/CrushFTPDrive/win_service.jar") >= 0) {
        ...
        if (file.getCanonicalPath().indexOf("WebInterface") < 0)
          file = null; 
      } else if (x == 0 && (data.startsWith("HEAD ") || data.startsWith("GET ")) && (data.indexOf(" /WebInterface/") >= 0 || data.toUpperCase().indexOf(" /FAVICON.ICO") >= 0 || data.indexOf(" /.well-known/acme-challenge/") >= 0)) {
        if (data.toUpperCase().startsWith("HEAD "))
          headerOnly = true; 
        ...
        if (data.indexOf(" /.well-known/acme-challenge/") >= 0)
          theFile = "/WebInterface/" + Common.last(theFile); 
    ...
    if (file != null) {
      String request_file = Common.dots(file.getCanonicalPath().replace('\\', '/'));
    ...
    if (file.exists() && (mimes.getProperty(ext, "").toUpperCase().endsWith("HTML") || mimes.getProperty(ext, "").toUpperCase().endsWith("/CSS") || mimes.getProperty(ext, "").toUpperCase().endsWith("/JAVASCRIPT") || mimes.getProperty(ext, "").toUpperCase().endsWith("/X-JAVA-JNLP-FILE")) && theFile.toUpperCase().startsWith("/WEBINTERFACE/")) {
      ...
      if (ServerStatus.BG("email_reset_token")) {
        ...
        template = Common.replace_str(template, "javascript:showResetPanel();", String.valueOf(sessionHTTP.proxy) + "WebInterface/jQuery/reset.html");
      } 
    } 
    ...
    if (mimes.getProperty(ext, "").toUpperCase().endsWith("/HTML") && theFile.toUpperCase().startsWith("/WEBINTERFACE/")) {
      ...
      if (System.getProperty("crushftp.singleuser", "false").equals("true")) {
        template = Common.replace_str(template, "<!--##HEADER##-->", "<br/><center><font color='red' size='+4'><b>Server Maintenance Mode</b></font></center>\r\n" + ServerStatus.SG("login_header"));
      } else {
      ...
      if (sessionHTTP.server_item.getProperty("recaptcha_enabled", "false").equals("true"))
        if (sessionHTTP.server_item.getProperty("recaptcha_version", "2").equals("2")) {
      ...
      if (!sessionHTTP.server_item.getProperty("recaptcha_public_key", "").equals(""))
        if (sessionHTTP.server_item.getProperty("recaptcha_version", "2").equals("2")) {
          template = Common.replace_str(template, "<script type=\"text/javascript\" src=\"https://www.google.com/recaptcha/api/js/recaptcha_ajax.js\"></script>", "<script type=\"text/javascript\" src=\"https://www.google.com/recaptcha/api.js\"></script>");
      ...
      if (template.indexOf("var passwordRule =") >= 0) {
        ...
        if (sessionHTTP != null && sessionHTTP.thisSession != null) {
          user = sessionHTTP.thisSession.user;
        } else {
          user = new Properties();
        } 
