...
public class UploadFormsSearch {
  ...
  public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
    try {
      ...
      Vector allUploads = ServerStatus.thisObj.statTools.executeSqlQuery(sql_transfers, new Object[] { "UPLOAD", mmddyyyy.parse(params.getProperty("startDate", "1/1/2000 00:00:00")), mmddyyyy.parse(params.getProperty("endDate", "1/1/2100 00:00:00")) }false, params);
      ...
      for (int x = 0; x < summary.size(); x++) {
        ...
        for (int xx = 0; xx < allUploads.size(); xx++) {
          ...
          if (transfer.getProperty("session_rid", "").equals(sessions.getProperty("rid", ""))) {
            Vector transferMetas = ServerStatus.thisObj.statTools.executeSqlQuery(sql_meta, new Object[] { transfer.getProperty("rid") }, false, params);
