...
public class NewFiles {
  ...
  public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
    try {
          ...
          for (int x = 0; x < current_user_group_listing.size(); x++) {
            ...
            if (user_ok || usernames.size() <= 0) {
              ...
              while (keys.hasMoreElements()) {
                ...
                if (!(new File_U(path)).exists() && !(new File_U(pathOriginal.substring("FILE:".length()))).exists())
              ...
              if (validFiles.size() > 0) {
                ...
                for (int j = 0; j < validFiles.size(); j++) {
                  ...
                  the_line = Common.replace_str(the_line, "%the_file_size_formatted%", Common.format_bytes_short2(Long.parseLong(p.getProperty("size", "0"))));
      ...
      sb.append(Common.getXMLString(results, "results", "WebInterface/Reports/NewFiles.xsl"));
      Common.writeXMLObject(String.valueOf(System.getProperty("crushftp.backup")) + "backup/NewFileReportCache.XML", cachedDirListings, "cached_dir");
    } catch (Exception e) {
      Log.log("REPORT", 1, e);
    } 
  }
