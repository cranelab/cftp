  ...
  public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
    try {
      ...
      for (int x = 0; x < summary.size(); x++) {
        ...
        Vector transferMetas = ServerStatus.thisObj.statTools.executeSqlQuery(sql_meta, new Object[] { p.getProperty("rid") }, false, params);
        ...
        if (transferMetas.size() > 0) {
          Vector transfers = ServerStatus.thisObj.statTools.executeSqlQuery(sql_transfers, new Object[] { p.getProperty("rid") }, false, params);
          if (transfers != null)
