...
public class SharedSessionReplicated {
                ...
                try {
                  ...
                  while (true) {
                    ...
                    if (!allowed_ips2.equals("") && !Common.do_search(allowed_ips2, incoming_ip, false, 0) && allowed_ips2.indexOf(incoming_ip) < 0) {
                      String msg = "IP " + sock.getInetAddress().getHostAddress() + " was from an untrusted host and was denied replication control. Allowed IPs: " + allowed_ips2;
                      sock.close();
                    ...
                    Worker.startWorker((Runnable)new Object(this, sock, current_name));
                  } 
                } catch (Exception e) {
                  Log.log("SERVER", 0, e);
                  return;
  ...
  public static void receive(Properties p) {
    ...
    ServerStatus.thisObj.server_info.put("replicated_received_message_count", (new StringBuffer(String.valueOf(Integer.parseInt(ServerStatus.thisObj.server_info.getProperty("replicated_received_message_count", "0")) + 1))).toString());
    if (p.getProperty("action").equalsIgnoreCase("RESPONSE")) {
      ...
      try {
        ...
        DMZServer.last_prefs_time.put(p.getProperty("key"), (new StringBuffer(String.valueOf((new File_S(String.valueOf(System.getProperty("crushftp.prefs")) + "prefs_" + p.getProperty("key") + ".XML")).lastModified()))).toString());
      } catch (Exception e) {
        Log.log("SERVER", 0, e);
      } 
      ServerStatus.thisObj.server_info.put("replicated_write_prefs_count", (new StringBuffer(String.valueOf(Integer.parseInt(ServerStatus.thisObj.server_info.getProperty("replicated_write_prefs_count", "0")) + 1))).toString());
      ...
      ServerStatus.thisObj.server_info.put("replicated_job_changes_count", (new StringBuffer(String.valueOf(Integer.parseInt(ServerStatus.thisObj.server_info.getProperty("replicated_job_changes_count", "0")) + 1))).toString());
      ...
      ServerStatus.thisObj.server_info.put("replicated_user_changes_count", (new StringBuffer(String.valueOf(Integer.parseInt(ServerStatus.thisObj.server_info.getProperty("replicated_user_changes_count", "0")) + 1))).toString());
  ...
  public static void send(String id, String action, Object key, Object val) {
    try {
      ...
      Log.log("SERVER", 2, "SHAREDSESSION:" + id + ":" + action + ":" + key);
  ...
  public static void flushPendingJobSync() {
    try {
      ...
      } else if (ServerStatus.BG("replicate_jobs_sync")) {
