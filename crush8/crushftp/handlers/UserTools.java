...
public class UserTools {
  ...
  public Properties getUser(String serverGroup, String username, boolean flattenUser) {
    if (username.equals(""))
      return null; 
    initUserProvider();
    if (serverGroup.equals("@AutoDomain"))
    ...
    try {
      ...
      if (folders.length >= maxCount) {
        ...
        while (validItems.size() > 0 && validItems.size() >= maxCount) {
          File_S f = validItems.elementAt(0);
  ...
  public static void deleteUser(String serverGroup, String username, boolean replicate) {
    ...
    sync_vfs_cache(serverGroup, username, new Properties());
    removeUserFromGroups(serverGroup, username);
    removeUserFromInheritance(serverGroup, username);
  }
    ...
    if (allUsers) {
      for (int x = 0; x < users.size(); x++) {
        ...
        if (!username.equalsIgnoreCase("default")) {
          ...
          try {
            File_S f = new File_S(get_real_path_to_user(serverGroup, username));
      ...
      if (user.getProperty("username").equalsIgnoreCase(the_user) && (user.getProperty("auto_set_pass", "false").equals("true") || ServerStatus.thisObj.common_code.decode_pass(user.getProperty("password")).equals("-AUTO-SET-ON-LOGIN-") || ServerStatus.thisObj.common_code.encode_pass("-AUTO-SET-ON-LOGIN-", "SHA", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass("-AUTO-SET-ON-LOGIN-", "SHA512", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass("-AUTO-SET-ON-LOGIN-", "SHA256", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass("-AUTO-SET-ON-LOGIN-", "SHA3", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass("-AUTO-SET-ON-LOGIN-", "MD5", user.getProperty("salt", "")).equals(user.getProperty("password")))) {
        ...
        if (ServerStatus.thisObj.common_code.decode_pass(user.getProperty("password")).equals(the_password) || ServerStatus.thisObj.common_code.encode_pass(the_password, "SHA", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass(the_password, "SHA512", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass(the_password, "SHA256", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass(the_password, "SHA3", user.getProperty("salt", "")).equals(user.getProperty("password")) || ServerStatus.thisObj.common_code.encode_pass(the_password, "MD5", user.getProperty("salt", "")).equals(user.getProperty("password"))) {
          ...
          synchronized (userExpirePasswordLock) {
            ...
            put_in_user(serverGroup, the_user, "password_history", Common.getPasswordHistory(the_password, user.getProperty("password_history", ""), password_rules), true, true);
            ...
            try {
              Worker.startWorker(new Runnable(this, user_f, the_user_f, failure_count_f) {
                    ...
                    public void run() {
                      ...
                      Properties info = new Properties();
                      info.put("user", this.val$user_f);
                      info.put("user_info", this.val$user_f);
                      ServerStatus.thisObj.events6.doEventPlugin(info, event, null, items);
                    }
                  });
            } catch (Exception e) {
  ...
  public boolean validateMd4(String the_user, String the_password, String user_hashed_pass) throws Exception {
    ...
    if (!user_hashed_pass.toUpperCase().startsWith("MD4:")) {
      ...
      if (user_hashed_pass.equals("") || user_hashed_pass.startsWith("MD5:") || user_hashed_pass.startsWith("BCRYPT:") || user_hashed_pass.startsWith("SHA:") || user_hashed_pass.startsWith("SHA512:") || user_hashed_pass.startsWith("SHA256:") || user_hashed_pass.startsWith("SHA3:") || user_hashed_pass.startsWith("3CRYPT:") || user_hashed_pass.startsWith("MD5CRYPT:") || user_hashed_pass.startsWith("PBKDF2SHA256:") || user_hashed_pass.startsWith("MD5S2:") || user_hashed_pass.startsWith("SHA512CRYPT:")) {
  ...
  public static void addItem(String serverGroup, String username, String path, String name, String url, String type, Properties moreItems, boolean encrypted, String encrypted_class, boolean replicate) throws Exception {
    ...
    sync_vfs_cache(serverGroup, username, ut.getVirtualVFS(serverGroup, username));
  }
  ...
  public static void writeVFS(String serverGroup, String username, Properties virtual, boolean replicate) {
    up.writeVFS(serverGroup, username, (Properties)Common.CLONE(virtual));
    Properties p = new Properties();
    p.put("serverGroup", serverGroup);
    p.put("username", username);
    ...
    sync_vfs_cache(serverGroup, username, virtual);
  }
  ...
  public static Vector buildPublicKeys(String username, Properties user, String serverGroup) throws IOException {
    ...
    if (keyStr.trim().equalsIgnoreCase("DMZ") || keyStr.trim().startsWith("DMZ:"))
      if (Common.dmz_mode) {
        ...
        if (keyStr.trim().startsWith("DMZ:")) {
          ...
          try {
            String serverGroup2 = serverGroup;
            String username2 = username;
            if (serverGroup2.equals("@AutoDomain"))
              if (username2.indexOf("@") > 0) {
                String newLinkedServer = username2.split("@")[(username2.split("@")).length - 1];
                String newLinkedServer2 = Common.dots(newLinkedServer);
                newLinkedServer2 = newLinkedServer2.replace('/', '-').replace('\\', '-').replace('%', '-').replace(':', '-').replace(';', '-');
                if (newLinkedServer.equals(newLinkedServer2)) {
            ...
            Properties template_vfs = ut.getVirtualVFS(serverGroup2, "template");
            Properties p0 = (Properties)template_vfs.get("/internal");
            if (p0 == null)
            ...
            if (p0 != null) {
              ...
              Properties p1 = v.elementAt(0);
              action.put("preferred_port", (new StringBuffer(String.valueOf((new VRL(p1.getProperty("url"))).getPort()))).toString());
            } 
          } catch (Exception e) {
            Log.log("SERVER", 2, e);
    ...
    if (keyStr.toUpperCase().indexOf("SSH2 PUBLIC KEY") >= 0 || keyStr.indexOf(";;;") >= 0) {
      ...
      while ((s = br.readLine()) != null) {
        ...
        if (s.endsWith("/") || s.endsWith("\\")) {
          if ((new File_S(s)).exists()) {
            File_S[] files = (File_S[])(new File_S(s)).listFiles();
            if (files != null)
          ...
          if (!key_vrl.getProtocol().equalsIgnoreCase("FILE")) {
            ...
            try {
              ...
              while (v.size() > 0) {
                Properties p = v.remove(0);
                if (Long.parseLong(p.getProperty("size", "0")) < 512000L && !p.getProperty("name", "").startsWith("."))
                  if (p.getProperty("name", "").toUpperCase().startsWith(String.valueOf(simpleUsername.toUpperCase()) + "_") || p.getProperty("name", "").toUpperCase().equals(simpleUsername.toUpperCase()) || p.getProperty("name", "").toUpperCase().equals(String.valueOf(simpleUsername.toUpperCase()) + ".PUB"))
      ...
      if ((new File_S(data)).exists() && (new File_S(data)).isFile()) {
        ...
        if (!key_vrl.getProtocol().equalsIgnoreCase("FILE")) {
          ...
          try {
            ...
            keysVec.setElementAt(new String(baos_key.toByteArray(), "UTF8"), x);
          } catch (Exception e) {
            Log.log("SERVER", 1, e);
          } finally {
            try {
