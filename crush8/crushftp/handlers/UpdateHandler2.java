...
public class UpdateHandler2 {
  ...
  public boolean doSilentUpdate(boolean earlyAccess, String thisVersion, boolean webOnly) throws Exception {
    ...
    String homeF = (new File_S(home)).getCanonicalPath();
    ...
    if (!(new File_S(String.valueOf(home) + fname + "_new.zip")).exists()) {
      ...
      (new Thread(new Runnable(this, files, homeF, localStatus) {
            ...
            public void run() {
              for (int x = 0; x < this.val$files.size(); x++) {
                ...
                if ((new File_S(String.valueOf(this.val$homeF) + tmpPath)).exists()) {
                  p.put("localSize", (new StringBuffer(String.valueOf((new File_S(String.valueOf(this.val$homeF) + tmpPath)).length()))).toString());
                  p.put("localModified", (new StringBuffer(String.valueOf((new File_S(String.valueOf(this.val$homeF) + tmpPath)).lastModified()))).toString());
                  if (p.getProperty("type").equals("FILE"))
                    try {
                      p.put("localMd5", Common.getMD5(new FileInputStream(new File_S(String.valueOf(this.val$homeF) + tmpPath))).trim().toUpperCase().substring(24));
                    } catch (Exception e) {
                      Log.log("UPDATE", 0, e);
                    }  
                } 
      ...
      Log.log("UPDATE", 0, "Updating " + smallFiles + " small files, and " + largeFiles + " large files:" + Common.format_bytes_short2(this.updateMaxSize));
      if (this.stopNow)
        return false; 
    ...
    try {
      ...
      for (int x = 0; x < list.size(); x++) {
        ...
        URL uRL = ((File_S)list.elementAt(x)).toURI().toURL();
        p.put("url", uRL);
      ...
      while ((entry = zin.getNextEntry()) != null) {
        ...
        if (out != null) {
          ...
          if (!ext.equals("")) {
            ...
            ok2 = !(new File_S(String.valueOf(sourcePath) + path2)).exists();
          } 
        } 
