...
public class CurrentLogins {
  ...
  public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
    try {
      ...
      for (x = user_list.size() - 1; x >= 0; x--) {
        ...
        try {
          ...
          if (userHistory.get(user_info.getProperty("login_date_stamp", "")) == null) {
            ...
            Vector session_commands = (Vector)historyUser.get("session_commands");
            Vector new_session_commands = (Vector)user_info.get("session_commands");
            for (int xx = 0; xx < new_session_commands.size(); xx++) {
              if (session_commands.indexOf(new_session_commands.elementAt(xx)) < 0)
                session_commands.add(new_session_commands.elementAt(xx)); 
            } 
      ...
      sb.append(common_code.getXMLString(si2, "results", "WebInterface/Reports/CurrentLogins.xsl"));
    } catch (Exception e) {
      Log.log("REPORT", 1, e);
    } 
  }
