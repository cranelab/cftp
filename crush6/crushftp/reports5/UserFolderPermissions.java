...
public class UserFolderPermissions {
  ...
  class sorter implements Comparator {
    ...
    final UserFolderPermissions this$0;
    
    sorter(UserFolderPermissions this$0) {
      this.this$0 = this$0;
      this.allItems = null;
      this.sort = null;
    }
  ...
  public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
    try {
        ...
        for (int x = 0; x < current_user_group_listing.size(); x++) {
          ...
          if (user_ok || usernames.size() <= 0) {
            ...
            if (!params.getProperty("linked_vfs_users", "").equals("")) {
              ...
              for (int j = 0; j < vfs_users.length && !linked_vfs_ok; j++) {
                for (int xxx = 0; xxx < linked_vfs.size() && !linked_vfs_ok; xxx++) {
                  if (vfs_users[j].trim().equalsIgnoreCase(linked_vfs.elementAt(xxx).toString().trim()))
                    linked_vfs_ok = true; 
                } 
              } 
            } else {
      ...
      if (users.size() > 0)
        sb.append(ServerStatus.thisObj.common_code.getXMLString(results, "results", "WebInterface/Reports/UserFolderPermissions.xsl")); 
    } catch (Exception e) {
      Log.log("REPORT", 1, e);
    } 
  }
