...
public class TransferSpeedometer implements Runnable {
  ...
  public TransferSpeedometer(ServerSession theSession, RETR_handler retr, STOR_handler stor) {
    ...
    if (!ipLimited(theSession.uiSG("user_ip"), ServerStatus.SG("bandwidth_immune_ips"))) {
  ...
  public void run() {
    try {
      ...
      while ((this.stor != null && this.stor.active && this.stor.the_dir.equals(this.the_dir)) || (this.retr != null && this.retr.active && this.retr.the_dir.equals(this.the_dir))) {
        Thread.sleep(this.sleep_interval);
        calcCurrent();
        calcOverall();
        if ((this.max_speed > 0 && ServerStatus.calc_server_speeds(this.theSession.uiSG("user_name")) >= this.max_speed * 1.0D) || (this.stor != null && this.max_server_upload_speed > 0L && ServerStatus.calc_server_up_speeds(null) > this.max_server_upload_speed * 0.93D) || (this.retr != null && this.max_server_download_speed > 0L && ServerStatus.calc_server_down_speeds(null) > this.max_server_download_speed * 0.93D) || this.theSession.uiBG("pause_now")) {
          if (this.stor != null)
