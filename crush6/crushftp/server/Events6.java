...
public class Events6 {
  ...
  public synchronized void process(String event_type, Properties fileItem1, Properties fileItem2, Vector the_events, ServerSession theSession) {
    ...
    for (int x = 0; x < the_events.size(); x++) {
      ...
      if ((event.getProperty("event_user_action_list", "").indexOf("(connect)") >= 0 && event_type.equals("LOGIN")) || (
        ...
        event.getProperty("event_user_action_list", "").indexOf("(upload)") >= 0 && event_type.equals("RENAME")) || (
  ...
  private void updateTracker(String id, String event_type, Properties event, Properties fileItem1, Properties fileItem2, ServerSession theSession) {
    ...
    if (fileItem1 != null) {
      if (event_type.equals("UPLOAD")) {
        ...
        for (int x = uploads.size() - 1; x >= 0 && !found; x--) {
          ...
          if (p.getProperty("url", "").equals(fileItem1.getProperty("url", ""))) {
        ...
        int xx;
        for (xx = downloads.size() - 1; xx >= 0; xx--) {
          Properties p = downloads.elementAt(xx);
          if (p.getProperty("url", "").equalsIgnoreCase(fileItem1.getProperty("url", "")))
            downloads.removeElementAt(xx); 
        } 
        for (xx = uploads.size() - 1; xx >= 0; xx--) {
        ...
        for (xx = 0; xx < uploads.size(); xx++) {
          Properties p = uploads.elementAt(xx);
          if (p.getProperty("url", "").equalsIgnoreCase(fileItem2.getProperty("url", ""))) {
            p.put("url", fileItem1.getProperty("url", ""));
            p.put("url_2", fileItem2.getProperty("url", ""));
            p.put("the_file_name", fileItem1.getProperty("the_file_name", ""));
            p.put("the_file_name_2", fileItem2.getProperty("the_file_name", ""));
  ...
  public synchronized void checkEventsNow() {
    String lastName = "";
    int x;
    for (x = 0; x < this.eventRunQueue.size(); x++) {
      try {
        Properties eventRun = this.eventRunQueue.elementAt(x);
        ServerSession theSession = (ServerSession)eventRun.get("session");
        ...
        long lastActivity = Long.parseLong(SharedSession.find("crushftp.usernames.activity").getProperty(eventRun.getProperty("id"), "0"));
          ...
          if (Long.parseLong(eventRun.getProperty("timeout", "0")) < System.currentTimeMillis()) {
            ...
            if (p == null || !p.getProperty("usedByNow_" + event.getProperty("id"), "false").equals("true")) {
              ...
              if (items.size() > 0 || event.getProperty("event_user_action_list", "").indexOf("(connect)") >= 0 || event.getProperty("event_user_action_list", "").indexOf("(site)") >= 0 || event.getProperty("event_user_action_list", "").indexOf("(welcome)") >= 0 || event.getProperty("event_user_action_list", "").indexOf("(error)") >= 0)
                runEvent(event, theSession, items); 
            } 
          } 
        ...
        if (doEventType != null) {
          ...
          for (int xx = items.size() - 1; xx >= 0; xx--) {
            ...
            Log.log("EVENT", 2, "Checking event item:" + p);
  ...
  public int countConnectedUsers(ServerSession this_user) {
    int num_users = 0;
    for (int x = ServerStatus.siVG("user_list").size() - 1; x >= 0; x--) {
      try {
        Properties p = ServerStatus.siVG("user_list").elementAt(x);
        if (((ServerSession)p.get("session")).getId().equalsIgnoreCase(this_user.getId())) {
          num_users++;
          Log.log("EVENT", 2, "Found similar user:" + num_users + ":" + ((ServerSession)p.get("session")).getId());
        } 
  ...
  public void runEvent(Properties event, ServerSession the_user, Vector items) throws Exception {
    ...
    Thread t = new Thread(new Runnable(this, items, event, the_user) {
          final Events6 this$0;
          
          private final Vector val$items;
          
          private final Properties val$event;
          
          private final ServerSession val$the_user;
            ...
            while (keys.hasMoreElements()) {
              ...
              if (this.val$event.getProperty("event_action_list", "").indexOf("(send_email)") >= 0) {
                ...
                if (groupedItems.size() > 0 || this.val$event.getProperty("event_user_action_list", "").indexOf("(connect)") >= 0 || this.val$event.getProperty("event_user_action_list", "").indexOf("(site)") >= 0 || this.val$event.getProperty("event_user_action_list", "").indexOf("(welcome)") >= 0 || this.val$event.getProperty("event_user_action_list", "").indexOf("(error)") >= 0)
                  this.this$0.doEventEmail(this.val$event, this.val$the_user, groupedItems); 
              } 
              if (this.val$event.getProperty("event_action_list", "").indexOf("(run_plugin)") >= 0)
                this.this$0.doEventPlugin(this.val$event, this.val$the_user, groupedItems); 
            } 
          }
        });
    t.setName(String.valueOf(Thread.currentThread().getName()) + ":Running event:" + event.getProperty("name"));
    t.start();
    if (!ServerStatus.BG("event_asynch")) {
  ...
  public void doEventEmail(Properties event, ServerSession the_user, Vector items) {
    ...
    try {
      ...
      while (body.toUpperCase().indexOf("<LINE>") >= 0 && body.toUpperCase().indexOf("</LINE>") >= 0) {
        ...
        for (int i = 0; i < items.size(); i++) {
          ...
          the_line = Common.replace_str(the_line, "%the_file_size_formatted%", Common.format_bytes_short(Long.parseLong(p.getProperty("the_file_size", "0"))));
          ...
          the_line = ServerStatus.thisObj.change_vars_to_values(the_line, the_user);
      ...
      body = ServerStatus.thisObj.change_vars_to_values(body, the_user);
      Properties form_email = Common.buildFormEmail(ServerStatus.server_settings, the_user.uiVG("lastUploadStats"));
      ...
      subject = replace_line_variables(items, subject, the_user);
      subject = ServerStatus.thisObj.change_vars_to_values(subject, the_user);
      subject = Common.replaceFormVariables(form_email, subject);
      to = replace_line_variables(items, event.getProperty("to"), the_user).trim();
      to = ServerStatus.thisObj.change_vars_to_values(to, the_user);
      to = Common.replaceFormVariables(form_email, to);
      cc = replace_line_variables(items, event.getProperty("cc"), the_user).trim();
      cc = ServerStatus.thisObj.change_vars_to_values(cc, the_user);
      cc = Common.replaceFormVariables(form_email, cc);
      bcc = replace_line_variables(items, event.getProperty("bcc"), the_user).trim();
      bcc = ServerStatus.thisObj.change_vars_to_values(bcc, the_user);
      bcc = Common.replaceFormVariables(form_email, bcc);
      from = replace_line_variables(items, event.getProperty("from"), the_user).trim();
      from = ServerStatus.thisObj.change_vars_to_values(from, the_user);
      ...
      String emailResult = Common.send_mail(ServerStatus.SG("discovered_ip"), to, cc, bcc, from, subject, body, ServerStatus.SG("smtp_server"), ServerStatus.SG("smtp_user"), ServerStatus.SG("smtp_pass"), ServerStatus.BG("smtp_ssl"), ServerStatus.BG("smtp_html"), null);
      ...
      Log.log("EVENT", 0, String.valueOf(LOC.G("Event:EMAIL")) + " " + ServerStatus.thisObj.change_vars_to_values(event.getProperty("name"), the_user) + ":" + to + ":" + e + "\r\n");
      Log.log("EVENT", 1, e);
      Properties m = new Properties();
  ...
  public String replace_line_variables(Vector items, String s, ServerSession the_user) {
    String s_line = "";
    try {
  ...
  public void doEventPlugin(Properties event, ServerSession the_user, Vector items) {
    try {
      Thread.currentThread().setName(String.valueOf(LOC.G("Event:PLUGIN")) + " " + event.getProperty("name"));
      Log.log("EVENT", 0, String.valueOf(LOC.G("Event:PLUGIN")) + " " + event.getProperty("name") + "\r\n");
      Properties p = new Properties();
      p.put("action", "event");
      p.put("server_settings", ServerStatus.server_settings);
      p.put("event", event);
      if (the_user != null)
        p.put("ServerSession", the_user); 
      if (the_user != null)
        p.put("ServerSessionObject", the_user); 
      if (the_user != null)
        p.put("user", the_user.user); 
      if (the_user != null)
        p.put("user_info", the_user.user_info); 
      p.put("items", items);
      ...
      Common.runPlugin(pluginName, p, subItem);
