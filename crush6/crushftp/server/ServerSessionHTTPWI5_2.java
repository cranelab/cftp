...
public class ServerSessionHTTPWI5_2 {
  public static File serveFile(ServerSessionHTTP5_2 sessionHTTP, Vector header, OutputStream original_os, boolean onlyGetFile, String downloadFilename) throws Exception {
    String data = "";
    ...
    if (theFile.toUpperCase().endsWith(".JPG") || theFile.toUpperCase().endsWith(".GIF") || theFile.toUpperCase().endsWith(".JAR") || theFile.toUpperCase().endsWith(".SWF"))
      acceptsGZIP = false; 
    sessionHTTP.updateMimes();
    Properties mimes = sessionHTTP.mimes;
    String ext = "";
    ...
    if (mimes.getProperty(ext, "").toUpperCase().endsWith("/HTML") && theFile.toUpperCase().startsWith("/WEBINTERFACE/")) {
      ...
      template = Common.replace_str(template, "<!--##CUSTOMSCRIPT##-->", ServerStatus.SG("login_custom_script"));
      ...
      template = Common.replace_str(template, "\"/WebInterface/custom.js\"", "\"/WebInterface/custom.js?random=" + System.currentTimeMillis() + "\"");
      template = Common.replace_str(template, "\"/WebInterface/custom.css\"", "\"/WebInterface/custom.css?random=" + System.currentTimeMillis() + "\"");
    ...
    if (!file.exists() && tempBytes.size() == 0) {
      ...
      if (downloadFilename != null) {
        ...
        sessionHTTP.write_command_http("Content-Disposition: attachment; filename=\"" + downloadFilename + "\"");
      } 
