  ...
  public void run() {
    try {
      if (System.getProperties().getProperty("os.name", "").toUpperCase().indexOf("NDOWS") >= 0) {
        ...
        try {
          RandomAccessFile in = new RandomAccessFile("update_list.txt", "r");
        ...
        while (loops++ < 5) {
          ...
          try {
            RandomAccessFile test = new RandomAccessFile("CrushFTP.jar", "rw");
            test.close();
            break;
        ...
        Thread.sleep(1000L);
        if (fileList.equals("")) {
          Process proc = Runtime.getRuntime().exec("cmd.exe /C update.bat".split(" "), (String[])null, new File(this.home));
          proc.waitFor();
        } else {
        ...
        Process proc = Runtime.getRuntime().exec("launchctl stop com.crushftp.CrushFTP".split(" "), (String[])null, new File(this.home));
        proc.waitFor();
        System.out.println("Daemon stopped.");
        Thread.sleep(1000L);
        proc = Runtime.getRuntime().exec("launchctl start com.crushftp.CrushFTP".split(" "), (String[])null, new File(this.home));
        proc.waitFor();
        System.out.println("Finished restarting daemon.");
      } else {
        System.out.println("Trying to restart the daemon...");
        this.home = String.valueOf((new File(this.home)).getCanonicalPath()) + "/";
