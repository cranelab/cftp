...
public class Applet2 extends Applet implements DropTargetListener {
  ...
  public void init() {
    ...
    updateRam();
    System.out.println();
    System.out.println("C 3.1.14 Initialized. " + Common.format_bytes_short(this.availableRam));
    setBackground(parseColorStr(getParameter("background-color")));
    appletContext = getAppletContext();
    (new Thread(new Runnable(this) {
          final Applet2 this$0;
    ...
    (new Thread(new Runnable(this) {
          ...
          public void run() {
            ...
            while (true) {
              ...
              synchronized (Applet2.log_lock) {
                while (Applet2.log.size() > 10000)
                  Applet2.log.remove(0); 
              } 
            } 
          }
        })).start();
          ...
          public void run() {
            try {
              ...
              if (!badJava && Common.machine_is_x() && this.val$p.getProperty("DIRECTORIES_ONLY", "").equals("true")) {
                ...
                returnVal = chooser.showOpenDialog(Applet2.thisObj);
  ...
  public void doUpload(Properties p) {
    ...
    (new Thread(new Runnable(this, uploader) {
          ...
          private final Uploader2 val$uploader;
  ...
  public void doDownload(Properties p) {
    ...
    (new Thread(new Runnable(this, downloader) {
          ...
          private final Downloader2 val$downloader;
  ...
  public void doLog(Properties p) {
    ...
    synchronized (log_lock) {
      int x = 0;
      while (true) {
        if ((((x < 1000) ? 1 : 0) & ((log.size() > 0) ? 1 : 0)) == 0) {
