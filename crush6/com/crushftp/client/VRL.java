...
public class VRL {
  ...
  public VRL(String url2) {
    ...
    if (Common.machine_is_windows() && url2.length() > 7 && url2.startsWith("file:/") && url2.charAt(7) == ':')
      url2 = url2.replace('\\', '/'); 
    if (url2.length() > 11 && url2.indexOf(":", 11) > 0 && url2.indexOf(":", 11) < url2.indexOf("@")) {
    ...
    this.url = url2;
    if (this.url.indexOf(":") > 2 || this.url.toUpperCase().startsWith("S3:") || this.url.toUpperCase().startsWith("GDRIVE:")) {
  ...
  public String getUsername() {
    return (this.u.getUserInfo() == null) ? "" : vrlDecode(Common.url_decode(this.u.getUserInfo().substring(0, this.u.getUserInfo().indexOf(":"))));
  }
