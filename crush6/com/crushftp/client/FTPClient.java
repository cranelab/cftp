...
public class FTPClient extends GenericClient {
  ...
  public String login2(String username, String password, String clientid) throws Exception {
    ...
    if (this.config.getProperty("secure", "false").equals("true") && !this.config.getProperty("implicit", "false").equals("true")) {
      ...
      SSLSocketFactory factory = (new Common()).getSSLContext(!this.config.getProperty("keystore_path", "").equals("") ? this.config.getProperty("keystore_path") : "builtin", this.config.getProperty("trustore_path", "builtin"), Common.encryptDecrypt(this.config.getProperty("keystore_pass"), false), Common.encryptDecrypt(this.config.getProperty("key_pass"), false), "TLS", !this.config.getProperty("keystore_path", "").equals(""), this.config.getProperty("acceptAnyCert", "true").equalsIgnoreCase("true")).getSocketFactory();
      SSLSocket ss = (SSLSocket)factory.createSocket(this.sock, u.getHost(), (u.getPort() == -1) ? 21 : u.getPort(), false);
      Common.configureSSLTLSSocket(ss, System.getProperty("crushftp.tls_version_client", "SSLv2Hello,TLSv1,TLSv1.1,TLSv1.2"));
      Common.setEnabledCiphers(this.config.getProperty("disabled_ciphers", ""), ss, null);
      ss.startHandshake();
      this.sock = ss;
    ...
    if (this.config.getProperty("server_type").indexOf("NONSTOP") >= 0) {
      ...
      this.config.put("force_ascii", "true");
    } 
  ...
  public boolean mdtm(String path, long modified) throws Exception {
    if (this.config.getProperty("no_mdtm", "false").equals("true"))
      return false; 
    boolean ok = this.config.getProperty("mfmt_ok", "true").equalsIgnoreCase("true");
    try {
      if (ok) {
        send_data(213, "MFMT " + this.yyyyMMddHHmmss.format(new Date(modified)) + " " + path);
        ok = true;
      } 
    } catch (Exception e) {
    ...
    if (!ok)
      send_data(213, "MDTM " + this.yyyyMMddHHmmss.format(new Date(modified)) + " " + path); 
    return true;
  }
  ...
  public Properties stat(String path) throws Exception {
    ...
    try {
      ...
      while ((data = br.readLine()) != null) {
        if (data.startsWith("2") || data.trim().equals(""))
          continue; 
        ...
        if (data.toUpperCase().indexOf("TYPE=") >= 0) {
          ...
          for (int x = 0; x < parts.length - 1; x++) {
            if (parts[x].split("=")[0].trim().equalsIgnoreCase("Type")) {
              ...
              Date d = this.yyyyMMddHHmmss.parse(parts[x].split("=")[1].trim());
  ...
  protected InputStream download3(String path1, long startPos, long endPos, boolean binary) throws Exception {
    ...
    send_data(-1, "TYPE " + ((binary && this.config.getProperty("force_ascii", "false").equals("false")) ? "I" : "A"));
    if (startPos > 0L)
    ...
    if (this.config.getProperty("pasv", "true").equals("true")) {
      ...
      if (this.config.getProperty("secure", "false").equals("true"))
  ...
  public void addModified(Vector list) {
    for (int x = 0; x < list.size(); x++) {
  ...
  public Object getTransferSocket(boolean pasv) throws Exception {
        ...
        while (ss == null && loops++ < 1000) {
          ...
          try {
            ...
            if (startPort != 1025 && endPort != 65535)
              listen_port = curPort; 
            if (this.config.getProperty("secure", "false").equalsIgnoreCase("true")) {
              if (common_code == null)
              ...
              ServerSocketFactory ssf = (new Common()).getSSLContext(!this.config.getProperty("keystore_path", "").equals("") ? this.config.getProperty("keystore_path") : "builtin", this.config.getProperty("trustore_path", "builtin"), Common.encryptDecrypt(this.config.getProperty("keystore_pass"), false), Common.encryptDecrypt(this.config.getProperty("key_pass"), false), "TLS", !this.config.getProperty("keystore_path", "").equals(""), this.config.getProperty("acceptAnyCert", "true").equalsIgnoreCase("true")).getServerSocketFactory();
              ss = common_code.getServerSocket(ssf, listen_port, null, this.config.getProperty("disabled_ciphers", ""), false);
            } else {
    ...
    for (int x = 0; x < 4; x++) {
      try {
        tempSock = Common.getSocket("FTP", new VRL("ftp://" + ip + ":" + port + "/"), this.config.getProperty("use_dmz", "false"), "");
        break;
      } catch (IOException e) {
        if (x == 3)
          throw e; 
    ...
    if (this.config.getProperty("secure", "false").equalsIgnoreCase("true")) {
      SSLSocket ss = null;
      SSLSocketFactory factory = (new Common()).getSSLContext(!this.config.getProperty("keystore_path", "").equals("") ? this.config.getProperty("keystore_path") : "builtin", this.config.getProperty("trustore_path", "builtin"), Common.encryptDecrypt(this.config.getProperty("keystore_pass"), false), Common.encryptDecrypt(this.config.getProperty("key_pass"), false), "TLS", !this.config.getProperty("keystore_path", "").equals(""), this.config.getProperty("acceptAnyCert", "true").equalsIgnoreCase("true")).getSocketFactory();
      ss = (SSLSocket)factory.createSocket(tempSock, ip, port, true);
      Common.configureSSLTLSSocket(ss, System.getProperty("crushftp.tls_version_client", "SSLv2Hello,TLSv1,TLSv1.1,TLSv1.2"));
      Common.setEnabledCiphers(this.config.getProperty("disabled_ciphers", ""), ss, null);
      return ss;
    } 
