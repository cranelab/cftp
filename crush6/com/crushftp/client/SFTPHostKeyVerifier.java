...
public class SFTPHostKeyVerifier implements HostKeyVerification {
  ...
  public boolean verifyHost(String host, SshPublicKey key) throws SshException {
    ...
    synchronized (knownHostLock) {
      ...
      try {
        if (!(new File(this.knownHostFile)).exists())
          (new FileOutputStream(this.knownHostFile)).close(); 
        br = new BufferedReader(new InputStreamReader(new FileInputStream(this.knownHostFile)));
        String line = "";
        ...
        if (this.addNewHost) {
          ...
          RandomAccessFile out = new RandomAccessFile(this.knownHostFile, "rw");
          try {
            if (out.length() > 0L) {
