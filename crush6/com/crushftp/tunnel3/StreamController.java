...
public class StreamController implements Runnable {
        ...
        if (this.tunnel.getProperty("reverse", "false").equals("false") || this.allowReverseMode) {
          try {
            ...
            if (ftp) {
              ...
              process(ssProxyControl.accept(), this.tunnel.getProperty("destIp"), Integer.parseInt(this.tunnel.getProperty("destPort")), 0);
              ssProxyControl.close();
              StreamFTP ftpp = new StreamFTP(this.sc);
              ftpp.proxyNATs(control, proxy);
              msg("Tunnel3:Started FTP NAT:control=" + control + " proxy=" + proxy);
            } else {
              process(proxy, this.tunnel.getProperty("destIp"), Integer.parseInt(this.tunnel.getProperty("destPort")), 0);
            } 
        ...
        Worker.startWorker(new Runnable(this, command, c) {
              ...
              public void run() {
                try {
                  ...
                  Stream st = new Stream(sock, this.val$c.id, this.this$0.sc, this.this$0.tunnel);
  ...
  public void process(Socket sock, String host, int port, int id2) throws Exception {
    if (id2 == 0)
    ...
    this.out_queue_commands.addElement(this.sc.makeCommand(id2, "CONNECT:" + host + ":" + port));
    Stream st = new Stream(sock, id2, this.sc, this.tunnel);
