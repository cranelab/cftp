...
public class GUI extends JPanel {
  ...
  public String doLogin(StringBuffer sb) throws Exception {
    ...
    urlc.setUseCaches(false);
    urlc.setDoOutput(true);
    urlc.getOutputStream().write(("command=login&username=" + System.getProperty("crushtunnel.remote.user") + "&password=" + System.getProperty("crushtunnel.remote.pass")).getBytes("UTF8"));
    urlc.getResponseCode();
    String cookies = urlc.getHeaderField("Set-Cookie");
    String ca = null;
    if (cookies.indexOf("CrushAuth=") >= 0)
  ...
  public void startStopTunnel(Properties p) {
    if (p.get("tunnel") == null) {
      ...
      if (System.getProperty("crushtunnel.launchbrowser", "").equalsIgnoreCase("true"))
        try {
          Desktop.getDesktop().browse(new URI("http://127.0.0.1:55555/"));
        } catch (Exception e) {
          e.printStackTrace();
        }  
    } else {
