...
public class SMTPTransport extends Transport {
  ...
  public synchronized String getLocalHost() {
    ...
    if (this.localHostName == null || this.localHostName.length() <= 0)
      this
        .localHostName = this.session.getProperty("mail." + this.name + ".localaddress"); 
    try {
      if (this.localHostName == null || this.localHostName.length() <= 0) {
  ...
  public synchronized String getAuthorizationId() {
    if (this.authorizationID == "UNKNOWN")
      this
        .authorizationID = this.session.getProperty("mail." + this.name + ".sasl.authorizationid"); 
  ...
  public synchronized String getNTLMDomain() {
    if (this.ntlmDomain == "UNKNOWN")
      this
        .ntlmDomain = this.session.getProperty("mail." + this.name + ".auth.ntlm.domain"); 
  ...
  public synchronized boolean isSSL() {
    return this.serverSocket instanceof javax.net.ssl.SSLSocket;
  }
  ...
  protected synchronized boolean protocolConnect(String host, int port, String user, String password) throws MessagingException {
    ...
    try {
      ...
      if (this.useStartTLS || this.requireStartTLS)
        if (this.serverSocket instanceof javax.net.ssl.SSLSocket) {
  ...
  private boolean authenticate(String user, String passwd) throws MessagingException {
    ...
    while (st.hasMoreTokens()) {
      ...
      if (mechs == this.defaultAuthenticationMechanisms) {
        ...
        boolean disabled = PropUtil.getBooleanSessionProperty(this.session, dprop, 
            !a.enabled());
        if (disabled) {
          if (this.logger.isLoggable(Level.FINE))
        ...
        if (this.resp != 235) {
          ...
          if (thrown != null) {
            ...
            if (thrown instanceof Exception)
              throw new AuthenticationFailedException(SMTPTransport.this
                  .getLastServerResponse(), (Exception)thrown); 
  ...
  private class LoginAuthenticator extends Authenticator {
    ...
    void doAuth(String host, String authzid, String user, String passwd) throws MessagingException, IOException {
      this.resp = SMTPTransport.this.simpleCommand(
  ...
  private class DigestMD5Authenticator extends Authenticator {
    ...
    void doAuth(String host, String authzid, String user, String passwd) throws MessagingException, IOException {
      ...
      assert md5 != null;
      byte[] b = md5.authClient(host, user, passwd, SMTPTransport.this.getSASLRealm(), SMTPTransport.this
          .getLastServerResponse());
  ...
  private class NtlmAuthenticator extends Authenticator {
    ...
    String getInitialResponse(String host, String authzid, String user, String passwd) throws MessagingException, IOException {
      this
        .ntlm = new Ntlm(SMTPTransport.this.getNTLMDomain(), SMTPTransport.this.getLocalHost(), user, passwd, SMTPTransport.this.logger);
      this.flags = PropUtil.getIntProperty(SMTPTransport.this
          .session.getProperties(), "mail." + SMTPTransport.this
  ...
  private boolean sasllogin(String[] allowed, String realm, String authzid, String u, String p) throws MessagingException {
    ...
    if (this.saslAuthenticator == null)
      try {
        ...
        this.saslAuthenticator = (SaslAuthenticator)c.newInstance(new Object[] { this, this.name, this.session


              
              .getProperties(), this.logger, serviceHost });
      } catch (Exception ex) {
    ...
    try {
      if (this.serverSocket != null) {
        ...
        if (this.quitWait) {
          ...
          if (resp != 221 && resp != -1 && this.logger
  ...
  private boolean convertTo8Bit(MimePart part) {
    ...
    try {
      if (part.isMimeType("text/*")) {
        ...
        if (enc != null && (enc.equalsIgnoreCase("quoted-printable") || enc
  ...
  protected void rcptTo() throws MessagingException {
    ...
    } else if (this.reportSuccess || (sendPartial && (invalid
      .size() > 0 || validUnsent.size() > 0))) {
  ...
  protected void startTLS() throws MessagingException {
    ...
    try {
      this.serverSocket = SocketFetcher.startTLS(this.serverSocket, this.host, this.session
          .getProperties(), "mail." + this.name);
      initStreams();
    } catch (IOException ioex) {
      closeConnection();
  ...
  private void initStreams() throws IOException {
    ...
    this
      .traceInput = new TraceInputStream(this.serverSocket.getInputStream(), this.traceLogger);
    this.traceInput.setQuote(quote);
    this
      .traceOutput = new TraceOutputStream(this.serverSocket.getOutputStream(), this.traceLogger);
  ...
  public boolean supportsExtension(String ext) {
    return (this.extMap != null && this.extMap
      .get(ext.toUpperCase(Locale.ENGLISH)) != null);
  }
  
  public String getExtensionParameter(String ext) {
    return (this.extMap == null) ? null : this.extMap
      .get(ext.toUpperCase(Locale.ENGLISH));
  }
