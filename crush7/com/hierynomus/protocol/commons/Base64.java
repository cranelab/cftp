...
public class Base64 {
  ...
  public static byte[] decode(byte[] source, int off, int len, int options) throws IOException {
    ...
    if (off < 0 || off + len > source.length)
      throw new IllegalArgumentException(String.format(
            "Source array with length %d cannot have offset of %d and process %d bytes.", new Object[] { Integer.valueOf(source.length), Integer.valueOf(off), 
              Integer.valueOf(len) })); 
    if (len == 0)
      return new byte[0]; 
    if (len < 4)
    ...
    for (i = off; i < off + len; i++) {
      ...
      if (sbiDecode >= -5) {
        ...
        throw new IOException(
            String.format("Bad Base64 input character '%c' in array position %d", new Object[] { Byte.valueOf(source[i]), Integer.valueOf(i) }));
      } 
    } 
  ...
  public static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws IOException {
    ...
    if (off + len > source.length)
      throw new IllegalArgumentException(String.format(
            "Cannot have offset of %d and length of %d with array of length %d", new Object[] { Integer.valueOf(off), Integer.valueOf(len), Integer.valueOf(source.length) })); 
    if ((options & 0x2) > 0) {
      ByteArrayOutputStream baos = null;
