...
public class IdlerKiller implements Runnable {
  ...
  SessionCrush calling_session = null;
  ...
  public IdlerKiller(SessionCrush calling_session, long last_activity, long timeout, Thread the_thread) {
  ...
  public void run() {
    try {
      while (!this.die_now) {
        ...
        while (System.currentTimeMillis() - start < this.sleep_interval && !this.die_now && !this.calling_session.uiBG("dieing") && this.calling_session.session_socks.size() > 0) {
          Thread.sleep(1000L);
        ...
        if (this.die_now || this.calling_session.uiBG("dieing") || this.calling_session.session_socks.size() == 0)
          break; 
        ...
        if ((new Date()).getTime() - this.last_activity > timeout2 * 1000L && this.enabled) {
          ...
          try {
            this.calling_session.do_kill(null);
          } catch (Exception exception) {}
          break;
        } 
      } 
