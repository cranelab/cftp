...
public class DesEncrypter {
  ...
  public String getHash(String key, boolean base64, String method, String salt_data, String user_salt) throws Exception {
    ...
    if (salt_data != null && !salt_data.equals("") && (new File(salt_data)).exists()) {
      salt_bytes = (byte[])salt_hash.get((new StringBuffer(String.valueOf((new File(salt_data)).lastModified()))).toString());
      if (salt_bytes == null) {
        RandomAccessFile raf = new RandomAccessFile(salt_data, "r");
        salt_bytes = new byte[(int)raf.length()];
        raf.read(salt_bytes);
        raf.close();
        salt_hash.put((new StringBuffer(String.valueOf((new File(salt_data)).lastModified()))).toString(), salt_bytes);
      } 
    } 
    ...
    SHA3.DigestSHA3 md3 = null;
    if (method.equals("SHA3")) {
      md = MessageDigest.getInstance("MD5");
      md3 = new SHA3.DigestSHA3(224);
    } else {
    ...
    if (user_salt.trim().length() > 0) {
      ...
      md3.update(key.getBytes(System.getProperty("crushftp.hash.encoding", "UTF8")));
    } else {
      md.update(key.getBytes(System.getProperty("crushftp.hash.encoding", "UTF8")));
    } 
    if (salt_bytes.length > 0)
      if (md3 != null) {
        md3.update(salt_bytes);
      } else {
    ...
    if (method.equals("SHA3"))
      return Hex.toHexString(md3.digest()).toLowerCase(); 
    if (base64)
  ...
  public String encrypt(String str, String method, boolean base64, String salt) {
    if (method.equals("SHA") || str.startsWith("SHA:") || method.equals("SHA512") || str.startsWith("SHA512:") || method.equals("SHA3") || str.startsWith("SHA3:") || method.equals("MD5") || str.startsWith("MD5:") || str.startsWith("MD5S2:") || method.equals("MD5S2") || method.equals("MD4") || str.startsWith("MD4:"))
      try {
        if (str.startsWith("SHA:") || str.startsWith("SHA512:") || str.startsWith("SHA3:") || str.startsWith("MD5:") || str.startsWith("MD5S2:") || str.startsWith("MD4:") || str.startsWith("CRYPT3:") || str.startsWith("BCRYPT:") || str.startsWith("MD5CRYPT:") || str.startsWith("SHA512CRYPT:"))
          return str; 
        if (base64)
