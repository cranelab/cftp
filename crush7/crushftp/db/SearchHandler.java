...
public class SearchHandler implements Runnable {
      ...
      if (FileClient.memCache) {
        ...
        for (int x = 0; x < root_items.size(); x++) {
          ...
          if (vrl.getProtocol().equalsIgnoreCase("file")) {
            String cpath = (new File(vrl.getPath())).getCanonicalPath().replace('\\', '/');
        ...
        this.thisSession.uVFS.getListing(this.listing, this.thisSession.uiSG("current_dir"), this.depth, 1000, true);
      } else {
  ...
  public static void buildEntry(Properties pp, VFS uVFS, boolean deleteOnly, boolean deleteItems) {
    ...
    boolean tika = (new File(String.valueOf(System.getProperty("crushftp.search")) + "tika-app.jar")).exists();
    while (listing.size() > 0 || status.length() == 0) {
      if (listing.size() > 0) {
        ...
        try {
          String contents = "";
          File f = new File((new VRL(ppp.getProperty("url"))).getPath());
          if (f.isFile() && tika) {
  ...
  public static byte[] getContents(File f, int max_kb) throws Exception {
    if (f.isDirectory())
      return new byte[0]; 
    Process proc = Runtime.getRuntime().exec((String.valueOf(System.getProperty("java.home")) + File.separator + "bin" + File.separator + "java;-jar;tika-app.jar;-t;" + f.getCanonicalPath()).split(";"), (String[])null, new File(System.getProperty("crushftp.search")));
  ...
  public static Properties findItem(Properties pp, VFS uVFS, Vector items, String root_dir) throws Exception {
    ...
    if (FileClient.memCache) {
      pp_canonical = (new File((new VRL(pp.getProperty("url"))).getPath())).getCanonicalPath();
    } else {
      pp_canonical = (new File(pp.getProperty("root_dir"))).getCanonicalPath();
    } 
  ...
  public static String getPreviewPath(Properties item, String size, int frame) throws Exception {
    ...
    if (item != null) {
      ...
      if (path.equalsIgnoreCase("/"))
        return null; 
      try {
        path = String.valueOf(PreviewWorker.getDestPath2(item.getProperty("url"))) + otherFile.getName() + "/";
      } catch (Exception e) {
        path = "./";
      } 
