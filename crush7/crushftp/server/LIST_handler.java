...
public class LIST_handler implements Runnable {
          ...
          try {
            ...
            if (!this.justStreamListData) {
              if (quota != -12345L && !ServerStatus.BG("hide_ftp_quota_log"))
                this.thisSession.not_done = this.thisSession.ftp_write_command("226-" + LOC.G("Quota space available") + ": " + Common.format_bytes_short2(quota)); 
  ...
  public void write150(long quota) throws Exception {
    ...
    if (quota != -12345L && !ServerStatus.BG("hide_ftp_quota_log"))
      this.thisSession.not_done = this.thisSession.ftp_write_command("150-" + LOC.G("Quota space available") + ": " + Common.format_bytes_short2(quota)); 
    if (this.thisSession.IG("max_download_amount") != 0)
  ...
  public static boolean checkName(Properties item, boolean fullPaths, boolean ignoreFilter, String file_filter, SessionCrush thisSession) throws Exception {
    ...
    if (ignoreFilter || Common.filter_check("L", item.getProperty("name"), filter_str)) {
      ...
      if ((item.getProperty("privs", "").indexOf("(view)") >= 0 && item.getProperty("privs", "").indexOf("*") <= 0) || !item.getProperty("name", "").equals("Icon\r")) {
        boolean is_root_dir = item.getProperty("root_dir", "").equals(thisSession.SG("root_dir"));
        ...
        boolean is_single_root = !thisSession.SG("root_dir").equals("/");
        if ((!is_invisible || (is_invisible && is_inherited && (!is_single_root || !is_root_dir))) && ((item.getProperty("privs", "").indexOf("(invisible)") >= 0 && item.getProperty("privs", "").indexOf("(inherited)") >= 0) || item.getProperty("privs", "").indexOf("(invisible)") < 0))
          return true; 
      } 
    } 
    return false;
  ...
  public static void generateLineEntry(Properties item, StringBuffer item_str, boolean makeupZipSize, String the_dir, boolean fullPaths, SessionCrush thisSession, boolean mlstFormat) {
    ...
    if (thisSession.BG("dos_ftp_listing")) {
      ...
      d = new Date(d.getTime() + (thisSession.IG("timezone_offset") * 1000 * 60 * 60));
