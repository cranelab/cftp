...
public class SSHDaemon extends Daemon {
  ...
  public SSHDaemon(Properties server_item) {
    ...
    System.setProperty("ssh.maxWindowSpace", (new StringBuffer(String.valueOf(1024 * Integer.parseInt(server_item.getProperty("window_space", "512"))))).toString());
  ...
  protected void configure(DaemonContext context) throws IOException {
    ...
    if (!this.server_item.getProperty("max_dh_size", "").trim().equals("") && !this.server_item.getProperty("max_dh_size", "").trim().equals("0"))
      this.sshContext.setMaximumDHGroupSize(Integer.parseInt(this.server_item.getProperty("max_dh_size", "1024"))); 
    ...
    try {
      ...
      String[] ciphers = this.server_item.getProperty("ssh_cipher_list", "aes128-ctr,3des-cbc,blowfish-cbc,arcfour128,arcfour").split(",");
      ...
      if (!ServerStatus.BG("ssh_sha1_kex_allowed")) {
        this.sshContext.supportedKeyExchanges().remove("diffie-hellman-group1-sha1");
        Log.log("SSH_SERVER", 0, "SSH KEX_DIFFIE_HELLMAN_GROUP1_SHA1 removed.");
      } 
      if (!ServerStatus.BG("ssh_sha1_group_kex_allowed"))
        this.sshContext.supportedKeyExchanges().remove("diffie-hellman-group-exchange-sha1"); 
      try {
      ...
      String macs_list = this.server_item.getProperty("ssh_mac_list", "hmac-md5,hmac-sha1,hmac-md5-96,hmac-sha1-96,hmac-sha256,hmac-sha2-256,hmac-sha256@ssh.com").toLowerCase();
      String[] macs = this.sshContext.supportedMacsCS().toArray();
      for (int i = 0; i < macs.length; i++) {
        String c = macs[i].trim().toLowerCase();
      ...
      if (this.server_item.getProperty("ssh_rsa_enabled", "true").equals("true") && !rsa_key.equals(""))
        if (Common.System2.containsKey("crushftp.keystores." + rsa_key.toUpperCase().replace('\\', '/'))) {
          ...
          try {
            this.sshContext.loadOrGenerateHostKey(new File(rsa_key), "ssh-rsa", 4096);
          } catch (Exception e) {
            try {
              this.sshContext.loadOrGenerateHostKey(new File(rsa_key), "ssh-rsa", 2048);
            } catch (Exception e1) {
      ...
      if (this.server_item.getProperty("ssh_dsa_enabled", "true").equals("true") && !dsa_key.equals(""))
        if (Common.System2.containsKey("crushftp.keystores." + dsa_key.toUpperCase().replace('\\', '/'))) {
          ...
          try {
            this.sshContext.loadOrGenerateHostKey(new File(dsa_key), "ssh-dss", 4096);
          } catch (Exception e) {
            Log.log("SERVER", 0, e);
            try {
              this.sshContext.loadOrGenerateHostKey(new File(dsa_key), "ssh-dss", 2048);
            } catch (Exception e1) {
              Log.log("SERVER", 0, e1);
      ...
      class$11.setFileSystemProvider(class$11 = Class.forName("crushftp.server.ServerSessionSSH"));
      if (class$12 == null)
        try {
      ...
      "sftp".addCommand((String)class$11, class$11 = Class.forName("crushftp.server.ServerSessionSSH"));
    } catch (Exception e) {
      Log.log("SSH_SERVER", 0, e);
    } 
