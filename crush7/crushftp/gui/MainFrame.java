  ...
  public MainFrame() {
    ...
    this.btnAuthenticateCrushftpFor.addActionListener(new ActionListener(this) {
          ...
          public void actionPerformed(ActionEvent arg0) {
            try {
              Runtime.getRuntime().exec(new String[] { "osascript", "-e", "do shell script \"" + (new File(String.valueOf(System.getProperty("crushftp.home")) + System.getProperty("crushftp.executable"))).getCanonicalPath() + "\" with administrator privileges" });
              Thread.sleep(1000L);
            } catch (Exception ee) {
              Log.log("SERVER", 0, ee);
    ...
    this.btnStopDaemon.addActionListener(new ActionListener(this) {
          ...
          public void actionPerformed(ActionEvent arg0) {
            ...
            try {
              ...
              if (Common.machine_is_windows()) {
                if (Common.test_elevate()) {
                  ...
                  proc = Runtime.getRuntime().exec(new String[] { (new File("./service/elevate.exe")).getCanonicalPath(), "-c", "-w", "net", "stop", "CrushFTP Server" }, (String[])null, new File("./service/"));
                } else {
                  proc = Runtime.getRuntime().exec(new String[] { "net", "stop", "CrushFTP Server" }, (String[])null, new File("./service/"));
                } 
              } else if (Common.machine_is_x()) {
                ...
                out.close();
                Common.exec(new String[] { "chmod", "+x", (new File("crushftp_exec_root.sh")).getCanonicalPath() });
                proc = Runtime.getRuntime().exec(new String[] { "osascript", "-e", "do shell script \"" + (new File("crushftp_exec_root.sh")).getCanonicalPath() + "\" with administrator privileges" });
              } 
            ...
            if ((new File("crushftp_exec_root.sh")).exists())
              (new File("crushftp_exec_root.sh")).delete(); 
            if ((new File("service/elevate.exe")).exists())
          ...
          public void actionPerformed(ActionEvent arg0) {
            ...
            try {
              ...
              if (Common.machine_is_windows()) {
                if (Common.test_elevate()) {
                  Common.writeFileFromJar("service/elevate.exe", "./", true);
                  proc = Runtime.getRuntime().exec(new String[] { (new File("./service/elevate.exe")).getCanonicalPath(), "-c", "-w", "net", "start", "CrushFTP Server" }, (String[])null, new File("./service/"));
                } else {
                  proc = Runtime.getRuntime().exec(new String[] { "net", "stop", "CrushFTP Server" }, (String[])null, new File("./service/"));
                } 
              } else if (Common.machine_is_x()) {
